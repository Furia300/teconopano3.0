...
function AdminTableSortHeader({
  ...
  return (
    <button
      type={type}
      className={cn(
        'inline-flex items-center gap-1 whitespace-nowrap rounded-md text-left transition-colors hover:text-[var(--fips-fg)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--fips-primary)]/20',
        active ? 'text-[var(--fips-fg)]' : 'text-[var(--fips-fg-muted)]',
        className,
      )}
      {...props}
    >
      ...
      {active ? (
        direction === 'asc' ? (
          <ArrowUp className="h-3.5 w-3.5 text-[var(--fips-secondary)]" aria-hidden />
        ) : (
          <ArrowDown className="h-3.5 w-3.5 text-[var(--fips-secondary)]" aria-hidden />
        )
      ) : (
        <ArrowUpDown className="h-3.5 w-3.5 text-[var(--fips-border-strong)]" aria-hidden />
      )}
    </button>
  )
}
...
function AdminTableCompanyCell({ className, initials, name, ...props }: AdminTableCompanyCellProps) {
  return (
    <div className={cn('flex items-center gap-2.5', className)} {...props}>
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-fips-blue-100)] text-[11px] font-semibold text-[var(--fips-secondary)]">
        {initials}
      </div>
      <span className="max-w-[220px] truncate font-medium text-[var(--fips-fg)]">{name}</span>
    </div>
  )
}

const statusToneClasses: Record<AdminTableStatusTone, string> = {
  success: 'bg-[var(--fips-success)]',
  warning: 'bg-[var(--fips-warning)]',
  danger: 'bg-[var(--fips-danger)]',
  muted: 'bg-[var(--fips-border-strong)]',
}
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isDropTarget
                      ? 'border-[var(--fips-secondary)]/40 bg-[var(--color-fips-blue-100)]/45'
                      : 'border-transparent hover:border-[var(--fips-border)] hover:bg-[var(--fips-surface-soft)]',
                  )}
                >
                  <span className="cursor-grab text-[var(--fips-fg-muted)] active:cursor-grabbing">
                  ...
                  <input
                    ...
                    className="h-4 w-4 rounded accent-[var(--fips-secondary)]"
                    aria-label={`Alternar coluna ${column.label}`}
                  />
                  <span className="flex-1 truncate text-[var(--fips-fg)]">{column.label}</span>
                  {checked ? <Check className="h-3.5 w-3.5 text-[var(--fips-success)]" aria-hidden /> : null}
                </div>
              )
            })}
          </div>
...
function AdminTablePagination({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-3 border-t border-[var(--fips-border)] px-6 py-4 lg:flex-row lg:items-center lg:justify-between',
        className,
      )}
      {...props}
    >
      <div className="flex items-center gap-2 text-sm text-[var(--fips-fg-muted)]">
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm text-[var(--fips-fg-muted)]">
