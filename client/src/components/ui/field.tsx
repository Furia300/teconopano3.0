  ...
  return (
    <label
      ...
      className={cn(
        'block font-semibold',
        density === 'compact'
          ? 'text-xs leading-4 text-[var(--fips-fg)]'
          : 'text-[0.95rem] leading-5 uppercase tracking-[0.02em] text-[var(--fips-fg-muted)]',
    ...
    >
      ...
      {required ? (
        <>
          <span aria-hidden className="ml-1 text-[var(--fips-danger)]">*</span>
...
const FieldHint = React.forwardRef<HTMLParagraphElement, FieldHintProps>(({ className, ...props }, ref) => {
  ...
  return (
    <p
      ref={ref}
      className={cn(
        'text-xs leading-4.5 text-[var(--fips-fg-muted)]',
        fieldTextOffsetClasses[density][inset],
        className,
      )}
      {...props}
...
const fieldMessageToneClasses: Record<NonNullable<FieldMessageProps['tone']>, string> = {
  default: 'text-[var(--fips-fg-muted)]',
  danger: 'text-[var(--fips-danger)]',
  success: 'text-[var(--fips-success-strong)]',
}
